...
def consume(message):
    ...
    # if user doesnt have anything in their list, dont let them consume
    if len(result) == 0:
...
"""
helper function for /consume command 
takes in message sent from user, check if the message is valid to consume food item
if food item is valid (valid name, valid servings), it will be removed from database
else it will not be removed, will prompt user to reenter (with appropriate error messages)
"""
